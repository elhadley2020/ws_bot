...
# ================= STATE =================
state = {"price":{p:pd.DataFrame(columns=["time","open","high","low","close"]) for p in INSTR},
         "last":{p:0 for p in INSTR},"pos":{p:0 for p in INSTR}}

# ================= UTILITIES =================
def add_ind(df):
    df['ema']=df['close'].ewm(span=EMA,adjust=False).mean()
    d=df['close'].diff(); g=d.clip(lower=0); l=-d.clip(upper=0)
    ag=g.ewm(alpha=1/RSI,adjust=False).mean(); al=l.ewm(alpha=1/RSI,adjust=False).mean()
...
def log_trade(pair,units,sig,price): pd.DataFrame([[datetime.now(),pair,units,sig,price]],
                      columns=["time","pair","units","signal","price"]).to_csv(LOG,mode="a",header=not os.path.exists(LOG),index=False)
...
async def place_order(session, pair, units, stop, tp):
    ...
    stop=round(float(stop),precision); tp=round(float(tp),precision)
    payload={"order":{"instrument":pair,"units":str(units),"type":"MARKET",
...
async def refresh_positions(session):
    """
    Updates state['pos'] by checking actual open positions.
    Allows new trades once previous positions are closed.
    """
...
# ================= CORE LOGIC =================
async def process(session, tick):
    pair = tick['instrument']
    await refresh_positions(session)  # Reset pos if trades closed

    bid = float(tick['bids'][0]['price']); ask=float(tick['asks'][0]['price']); price=(bid+ask)/2
    df=state['price'][pair]; now=pd.Timestamp.utcnow().floor("min")
    ...
    if not df.empty and df['time'].iloc[-1]==now:
        ...
        df=pd.concat([df,pd.DataFrame([[now,price,price,price,price]],columns=["time","open","high","low","close"])],ignore_index=True)
    state['price'][pair]=df.tail(200)

    if len(df)<ATR+10: return
    df=add_ind(df); sig=signal(df)
    if sig==0 or state['pos'][pair]!=0 or (datetime.utcnow().timestamp()-state['last'][pair])<COOLD: return

    nav=await get_nav(session); atr=df['atr'].iloc[-1]; units=calc_units(nav,atr)
    if units==0: return
    ...
    state['last'][pair]=datetime.utcnow().timestamp(); state['pos'][pair]=sig
    ...
    while True:
        try:
            async with aiohttp.ClientSession(headers=headers) as s:
                async with s.get(STREAM, params=params) as r:
                    print("Streaming connected...")
                    async for line in r.content:
                        if not line: continue
                        data=json.loads(line.decode("utf-8"))
                        if "bids" in data: await process(s,data)
